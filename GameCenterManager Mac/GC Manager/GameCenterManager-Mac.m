...
- (BOOL)checkGameCenterAvailability {
    ...
    if (!isGameCenterAPIAvailable) {
        NSDictionary *errorDictionary = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:@"GameKit Framework not available on this Mac. GameKit is only available on Macs with OS X 10.8.0 or higher.", @"GameCenter Unavailable", nil]
                                                                    forKeys:[NSArray arrayWithObjects:@"message", @"title", nil]];
                ...
                [leaderboardRequest loadScoresWithCompletionHandler:^(NSArray *scores, NSError *error) {
                    if (error == nil) {
                        if (scores.count > 0) {
                            ...
                            [playerDict setObject:[NSNumber numberWithInt:MAX(leaderboardRequest.localPlayerScore.value, savedHighScoreValue)]
                                           forKey:leaderboardRequest.localPlayerScore.category];
                            [plistDict setObject:playerDict
                                          forKey:[[GameCenterManager sharedManager] localPlayerId]];
...
- (void)saveAndReportAchievement:(NSString *)identifier percentComplete:(double)percentComplete shouldDisplayNotification:(BOOL)displayNotification {
    ...
    // Get the saved percent complete value - if it's nil, initialize it to 0, otherwise set it to its double value
    NSNumber *savedPercentComplete = [playerDict objectForKey:identifier];
    if (savedPercentComplete == nil) savedPercentComplete = [NSNumber numberWithDouble:0];
    ...
    if ([[GameCenterManager sharedManager] checkGameCenterAvailability] == YES) {
        ...
        // Set whether or not a GameCenter achievement banner should be displayed
        ...
        // Report the achievement to GameCenter
        [achievement reportAchievementWithCompletionHandler:^(NSError *error) {
            NSDictionary *dict = nil;
        ...
        // GameCenter is not available, just save the achievement for later
        [[GameCenterManager sharedManager] saveAchievementToReportLater:identifier percentComplete:percentComplete];
    }
}
    ...
    if (percentNumber && achievement) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([[self delegate] respondsToSelector:@selector(gameCenterManager:savedAchievement:)])
                [[self delegate] gameCenterManager:self savedAchievement:[NSDictionary dictionaryWithObjects:@[achievement, percentNumber]
                                                                                                     forKeys:@[@"achievement", @"percent complete"]]];
        });
    } else {
...
- (void)getChallengesWithCompletion:(void (^)(NSArray *challenges, NSError *error))handler {
    if ([[GameCenterManager sharedManager] checkGameCenterAvailability] == YES) {
        ...
        if (isGameCenterChallengeAPIAvailable == YES) {
            ...
            NSError *error = [NSError errorWithDomain:[NSString stringWithFormat:@"GKChallenge Class is not available. GKChallenge is only available on OS X 10.8.2 and higher."]
                                                 code:GCMErrorChallengeNotAvailable userInfo:nil];
            if ([[self delegate] respondsToSelector:@selector(gameCenterManager:error:)])
                [[self delegate] gameCenterManager:self error:error];
        }
    } else {
